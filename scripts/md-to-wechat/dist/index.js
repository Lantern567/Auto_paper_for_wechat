 ...
 */
function insertImagesIntoMarkdown(markdown, imagePaths) {
 ...
 */
async function injectMarkdown(page, markdown) {
    try {
 ...
 * 点击复制到公众号按钮并保存复制的内容
 */
async function copyToWechat(page, outputPath) {
    try {
 ...
 */
async function convertMarkdownToWechatHTML(markdown, imagePaths) {
    let browser = null;
    try {
...
async function main() {
    ...
    try {
        ...
        const outputPath = path.join(parsedPath.dir, `${parsedPath.name}_wechat.html`);
        // 8. 复制到公众号并保存
